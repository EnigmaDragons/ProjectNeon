...
*/
import { createCommonjsModule, defaultOnWarn, ensureArray as ensureArray$1, warnUnknownOptions, unwrapExports, getCjsExportFromNamespace, fseventsImporter, rollupInternal } from './rollup.js';
...
function mergeOptions(config, rawCommandOptions = { external: [], globals: undefined }, defaultOnWarnHandler = defaultOnWarn) {
    ...
    warnUnknownOptions(command, Object.keys(inputOptions).concat(Object.keys(outputOptions[0]).filter(option => option !== 'sourcemapPathTransform'), Object.keys(commandAliases), 'config', 'environment', 'plugin', 'silent', 'stdin'), 'CLI flags', warn, /^_$|output$|config/);
