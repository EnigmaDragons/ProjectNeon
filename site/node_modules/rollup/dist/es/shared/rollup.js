...
class Bundle {
    constructor(outputOptions, unsetOptions, inputOptions, pluginDriver, chunks) {
    ...
    async generate(isWrite) {
        ...
        try {
            ...
            await Promise.all(this.chunks.map(chunk => {
    ...
    assignChunkIds(inputBase, addons, bundle) {
        const entryChunks = [];
        const otherChunks = [];
        for (const chunk of this.chunks) {
...
pp$3.parseSubscripts = function(base, startPos, startLoc, noCalls) {
  ...
  while (true) {
    var element = this.parseSubscript(base, startPos, startLoc, noCalls, maybeAsyncArrow, shortCircuited);
...
pp$3.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, shortCircuited) {
  var optional = this.options.ecmaVersion >= 11 && this.eat(types.optionalChaining);
  ...
  if ((optional && this.type !== types.parenL && this.type !== types.backQuote) || computed || this.eat(types.dot)) {
    var node = this.startNodeAt(startPos, startLoc);
    node.object = base;
    ...
    if (this.options.ecmaVersion >= 11) {
      ...
      node$1.shortCircuited = !!shortCircuited;
    }
...
pp$9.readToken_question = function() { // '?'
  if (this.options.ecmaVersion >= 11) {
    ...
    if (next === 46) {
      var next2 = this.input.charCodeAt(this.pos + 2);
      if (next2 < 48 || next2 > 57) { return this.finishOp(types.optionalChaining, 2) }
    }
  }
...
class ExternalModule {
    constructor(options, id, moduleSideEffects) {
        this.options = options;
...
function getInteropBlock(dependencies, options, varOrConst) {
    ...
    return dependencies
        ...
        if (!(namedExportsMode && exportsDefault && options.interop))
            return null;
        if (exportsNames) {
...
function amd(magicString, { accessedGlobals, dependencies, exports, hasExports, indentString: t, intro, isEntryModuleFacade, namedExportsMode, outro, varOrConst, warn }, options) {
    ...
    const wrapperStart = `${amdOptions.define}(${params}function${_}(${args.join(`,${_}`)})${_}{${useStrict}${n}${n}`;
...
function umd(magicString, { dependencies, exports, hasExports, indentString: t, intro, namedExportsMode, outro, varOrConst, warn }, options) {
    ...
    const amdParams = (options.amd.id ? `'${options.amd.id}',${_}` : ``) +
...
class Module {
    constructor(graph, id, options, moduleSideEffects, syntheticNamedExports, isEntryPoint) {
        this.graph = graph;
        this.id = id;
        this.options = options;
        this.moduleSideEffects = moduleSideEffects;
...
class Chunk$1 {
    constructor(orderedModules, inputOptions, unsetOptions, modulesById) {
    ...
    static generateFacade(inputOptions, unsetOptions, modulesById, facadedModule, facadeName) {
        const chunk = new Chunk$1([], inputOptions, unsetOptions, modulesById);
        ...
        if (!chunk.dependencies.has(facadedModule.chunk) &&
    ...
    generateExports(options) {
        ...
        if (this.inputOptions.preserveModules || (this.facadeModule && this.facadeModule.isEntryPoint))
            this.exportMode = getExportMode(this, options, this.facadeModule.id, this.inputOptions.onwarn);
    }
    generateFacades() {
        const facades = [];
        ...
        for (const module of this.entryModules) {
            ...
            for (const facadeName of requiredFacades) {
                facades.push(Chunk$1.generateFacade(this.inputOptions, this.unsetOptions, this.modulesById, module, facadeName));
            }
        }
    ...
    generateIdPreserveModules(preserveModulesRelativeDir, options, existingNames, unsetOptions) {
        ...
        if (isAbsolute(id)) {
            ...
            path = relative(preserveModulesRelativeDir, `${dirname(sanitizedId)}/${renderNamePattern(pattern, 'output.entryFileNames', {
    ...
    getDynamicImportIds() {
        return Array.from(this.dynamicDependencies, chunk => chunk.id);
    }
    getExportNames() {
        return (this.sortedExportNames || (this.sortedExportNames = Object.keys(this.exportsByName).sort()));
    }
    getImportIds() {
        return Array.from(this.dependencies, chunk => chunk.id);
    }
    ...
    async render(options, addons, outputChunk, outputPluginDriver) {
        ...
        const magicString = finalise(this.renderedSource, {
            ...
            intro: addons.intro,
            isEntryModuleFacade: this.inputOptions.preserveModules ||
    ...
    getChunkDependencyDeclarations(options) {
        ...
        for (let exportName of this.getExportNames()) {
            ...
            if (exportName[0] === '*') {
                ...
                exportChunk = this.modulesById.get(exportName.substr(1));
        ...
        for (const dep of this.dependencies) {
            ...
            if (dep instanceof ExternalModule) {
                ...
                if (options.format === 'umd' || options.format === 'iife') {
                    globalName = getGlobalName(dep, options.globals, exportsNames || exportsDefault, this.inputOptions.onwarn);
                }
            }
    ...
    setIdentifierRenderResolutions(options) {
        ...
        deconflictChunk(this.orderedModules, this.dependencies, this.imports, usedNames, options.format, options.interop, this.inputOptions.preserveModules, syntheticExports);
    }
    setUpChunkImportsAndExportsForModule(module) {
        for (let variable of module.imports) {
            ...
            if (variable.module && variable.module.chunk !== this) {
                this.imports.add(variable);
                if (!(variable instanceof NamespaceVariable && this.inputOptions.preserveModules) &&
...
class ModuleLoader {
    ...
    async addEntryModules(unresolvedEntryModules, isUserDefined) {
        ...
        const loadNewEntryModulesPromise = Promise.all(unresolvedEntryModules.map(({ id, importer }) => this.loadEntryModule(id, true, importer))).then(entryModules => {
            ...
            for (let index = 0; index < entryModules.length; index++) {
                ...
                const existingIndexedModule = this.indexedEntryModules.find(indexedModule => indexedModule.module.id === entryModule.id);
    ...
    async resolveId(source, importer, skip = null) {
        return this.normalizeResolveIdResult(this.options.external(source, importer, false)
            ? false
            : await resolveId(source, importer, this.options.preserveSymlinks, this.pluginDriver, skip), importer, source);
    }
    ...
    async awaitLoadModulesPromise(loadNewModulesPromise) {
        this.latestLoadModulesPromise = Promise.all([
            loadNewModulesPromise,
            this.latestLoadModulesPromise
        ]);
        const getCombinedPromise = async () => {
    ...
    async fetchModule(id, importer, moduleSideEffects, syntheticNamedExports, isEntry) {
        const existingModule = this.modulesById.get(id);
        if (existingModule instanceof Module) {
            existingModule.isEntryPoint = existingModule.isEntryPoint || isEntry;
            return Promise.resolve(existingModule);
        }
        const module = new Module(this.graph, id, this.options, moduleSideEffects, syntheticNamedExports, isEntry);
    ...
    fetchResolvedDependency(source, importer, resolvedId) {
        if (resolvedId.external) {
            if (!this.modulesById.has(resolvedId.id)) {
                this.modulesById.set(resolvedId.id, new ExternalModule(this.options, resolvedId.id, resolvedId.moduleSideEffects));
            }
    ...
    async loadEntryModule(unresolvedId, isEntry, importer) {
        const resolveIdResult = await resolveId(unresolvedId, importer, this.options.preserveSymlinks, this.pluginDriver, null);
...
function getChunkAssignments(entryModules, manualChunkModules) {
    ...
    function assignEntryToStaticDependencies(entry, dynamicDependentEntryPoints, assignedEntry = entry) {
        ...
        for (const entry of entriesToCheck) {
            if (!superSet.has(entry)) {
                if (staticEntries.has(entry))
                    return false;
                const dynamicDependentEntryPoints = dynamicallyDependentEntryPointsByDynamicEntry.get(entry);
    ...
    return createChunks([...Object.keys(manualChunkModules), ...entryModules, ...dynamicEntryModules], assignedEntryPointsByModule);
}
...
class Graph {
    constructor(options, unsetOptions, watcher) {
        var _a, _b;
        this.options = options;
        this.unsetOptions = unsetOptions;
        ...
        if (this.options.preserveModules) {
            for (const module of this.modules) {
                if (module.isIncluded() ||
                    ...
                    const chunk = new Chunk$1([module], this.options, this.unsetOptions, this.modulesById);
            ...
            for (const chunkModules of this.options.inlineDynamicImports
                ...
                chunks.push(new Chunk$1(chunkModules, this.options, this.unsetOptions, this.modulesById));
            }
        }
        for (const chunk of chunks) {
            chunk.link();
...
const getPreserveModules = (config, inlineDynamicImports) => {
...
const getInput = (config, inlineDynamicImports) => {
    const configInput = config.input;
    const input = configInput == null ? [] : typeof configInput === 'string' ? [configInput] : configInput;
...
const getPreserveEntrySignatures = (config, unsetOptions, preserveModules) => {
...
function normalizeOutputOptions(config, inputOptions) {
    ...
    const outputOptions = {
        ...
        interop: (_g = config.interop) !== null && _g !== void 0 ? _g : true,
        ...
        strict: (_h = config.strict) !== null && _h !== void 0 ? _h : true,
...
const getFile = (config, inputOptions) => {
...
async function rollupInternal(rawInputOptions, watcher) {
    ...
    const result = {
        ...
        async generate(rawOutputOptions) {
            const { options: outputOptions, outputPluginDriver, unsetOptions } = getOutputOptionsAndPluginDriver(rawOutputOptions, graph.pluginDriver, inputOptions);
            const bundle = new Bundle(outputOptions, unsetOptions, inputOptions, outputPluginDriver, chunks);
...
function getInputOptions(rawInputOptions) {
    ...
    const { options, unsetOptions } = normalizeInputOptions(rawPlugins.reduce(applyOptionHook, rawInputOptions));
    normalizePlugins(options.plugins, ANONYMOUS_PLUGIN_PREFIX);
    return { options, unsetOptions };
}
function applyOptionHook(inputOptions, plugin) {
    if (plugin.options)
        return (plugin.options.call({ meta: { rollupVersion: version } }, inputOptions) ||
            inputOptions);
    return inputOptions;
}
...
function getOutputOptionsAndPluginDriver(rawOutputOptions, inputPluginDriver, inputOptions) {
    if (!rawOutputOptions) {
...
function getOutputOptions(inputOptions, rawOutputOptions, outputPluginDriver) {
...
export { createCommonjsModule, defaultOnWarn, ensureArray, fseventsImporter, getCjsExportFromNamespace, rollup, rollupInternal, unwrapExports, version, warnUnknownOptions, watch };
