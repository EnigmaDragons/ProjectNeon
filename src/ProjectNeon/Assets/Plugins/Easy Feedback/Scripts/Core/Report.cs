    ...
    public class Report
    {
        ...
        public ReportSection this[string sectionTitle]
        {
            get
            {
                ...
                Debug.LogError(
                    "Report does not contain a section with title \"" +
                    sectionTitle + "\""
                );
                return null;
            }
        ...
        public void AddSection(ReportSection section)
        {
            if (info.ContainsKey(section.Title))
            {
                ...
                Debug.LogError(
                    "Report already contains a section with title \"" +
                    section.Title + "\""
                );
                return;
            }
        ...
        public void RemoveSection(string title)
        {
            ...
            {
                Debug.LogWarning(
                    "Can not remove section \"" + title +
                    "\" because report does not contain a section with that name"
                );
                return;
            }
        ...
        public override string ToString()
        {
            ...
            var sections = info.Select(r => r.Value)
        ...
        public void AttachFile(FileAttachment file)
        {
            ...
            {
                Debug.LogError(
                    "Error attaching file: maximum attachment limit (" +
                    MAX_ATTACHMENTS + ") reached!"
                );
                return;
            }
        ...
        public void AddLabel(Label label)
        {
            ...
            {
                Debug.LogWarning(
                    "The report already has the label \"" + label.name + "\""
                );
                return;
            }
