	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag ( VertexOutput IN  ) : SV_Target
			{
				...
				float2 uv_ColorMask = IN.ase_texcoord7.xy * _ColorMask_ST.xy + _ColorMask_ST.zw;
				float4 tex2DNode2 = SAMPLE_TEXTURE2D( _ColorMask, sampler_ColorMask, uv_ColorMask );
				...
				float3 Albedo = ( ( 2.0 * lerpResult44 ) * _Color ).rgb;
				...
				float Occlusion = 1;
				float Alpha = ( _Color.a * tex2DNode11.a );
		...
		Pass
		{
			...
			half4 frag(VertexOutput IN  ) : SV_TARGET
			{
				...
				float2 uv_MainTex = IN.ase_texcoord2.xy * _MainTex_ST.xy + _MainTex_ST.zw;
				float4 tex2DNode11 = SAMPLE_TEXTURE2D( _MainTex, sampler_MainTex, uv_MainTex );
				float2 uv_ColorMask = IN.ase_texcoord2.xy * _ColorMask_ST.xy + _ColorMask_ST.zw;
				float4 tex2DNode2 = SAMPLE_TEXTURE2D( _ColorMask, sampler_ColorMask, uv_ColorMask );
				float4 lerpResult42 = lerp( ( half4(0.4980392,0.4980392,0.4980392,1) * tex2DNode11 ) , ( _Color1 * tex2DNode11 ) , tex2DNode2.r);
				float4 lerpResult43 = lerp( lerpResult42 , ( _Color2 * tex2DNode11 ) , tex2DNode2.g);
				float4 lerpResult44 = lerp( lerpResult43 , ( _Color3 * tex2DNode11 ) , tex2DNode2.b);
