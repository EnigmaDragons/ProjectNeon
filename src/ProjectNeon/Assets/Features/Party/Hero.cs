...
public class Hero
{
    ...
    {
        return WithEquipmentState(m,new EffectContext(m, new Single(m), Maybe<Card>.Missing(), ResourceQuantity.None, state.Party, state.PlayerState, state.Members, state.EnemiesById, state.PlayerCardZones, new UnpreventableContext(), new SelectionContext()));
    }
