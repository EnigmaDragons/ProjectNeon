...
public class MinionTeamRule : EncounterBuildingRule
{
    ...
    {
        if (ctx.PossibleEnemies.All(x => x.Tier == EnemyTier.Minion) || ctx.PossibleEnemies.All(x => x.Tier != EnemyTier.Minion))
            return ctx;
        return (ctx.SelectedEnemies.Length == 0 
               ...
               || (ctx.SelectedEnemies.Length > 0 
                   && ctx.SelectedEnemies.All(x => x.Tier == EnemyTier.Minion))
