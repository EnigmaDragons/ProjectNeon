    ...
    public void Apply(EffectContext ctx)
    {
        ...
        MessageGroup.ExtendCurrentGroup(new MultiplePayloads("inner effect", _data.Actions
            ...
            .Select(action => action.Type == CardBattleActionType.SpawnEnemy 
                ...
                : (object)new ApplyBattleEffect(false, action.BattleEffect, ctx.Source, ctx.Target, ctx.Card, ctx.XPaidAmount, ctx.PaidAmount, ctx.Preventions, ctx.IsReaction, ctx.Timing, ctx.DoubleDamage))
            .ToArray()));
    }
}