...
public sealed class EffectOnEvaded : Effect
{
    ...
    public EffectOnEvaded(bool isDebuff, int numberOfUses, int maxDurationTurns, ReactiveTriggerScope triggerScope, ReactionCardType reaction, IReadOnlyDictionary<int, Member> allMembers)
    {
        _isDebuff = isDebuff;
    ...
    public void Apply(Member source, Target target)
    {
        target.ApplyToAllConscious(m =>
            m.AddReactiveState(new ReactOnEvaded(_isDebuff, _numberOfUses, _maxDurationTurns, _triggerScope, _allMembers, m.MemberId, source, _reaction)));
    }
}

public sealed class ReactOnEvaded : ReactiveEffectV2Base
{
    public ReactOnEvaded(bool isDebuff, int numberOfUses, int maxDurationTurns, ReactiveTriggerScope triggerScope, IReadOnlyDictionary<int, Member> allMembers, int possessingMemberId, Member originator, ReactionCardType reaction)
